...
func process_client_connection(conn net.Conn) error {
	defer conn.Close()

	//--------删除上线 1--------
	if err := process_client_handshake(conn); err != nil {
		log.Println("Handshake error:", err)
		return err
	}
	//--------删除下线 1--------

	if err := process_client_request(conn); err != nil {
...
func process_client_handshake(conn net.Conn) error {
	buf := make([]byte, 2)
...
func process_client_request(conn net.Conn) error {
	defer conn.Close()
	buf := make([]byte, 256)
	n, err := conn.Read(buf)
	if err != nil {
...
func client() {
	...
	for {
		...
		go process_client_connection(conn)
	}
}
